...
function renderThumbMarkup(src?: string | null) {
  const trimmed = typeof src === "string" ? src.trim() : "";
  if (trimmed) {
    const safeSrc = trimmed.replace(/&/g, "&amp;").replace(/"/g, "&quot;");
    return `<img class="thumb" src="${safeSrc}" alt="" />`;
  }
...
function renderInventarioToDOM(arr: InventoryItem[]) {
  ...
  arr.forEach((item) => {
    const fotoValue = item.foto ?? "";
    const fotoMarkup = renderThumbMarkup(fotoValue);
    const hasFoto = Boolean(fotoValue.trim());
...
function bootstrapInventario() {
  ...
  if (Array.isArray(data) && data.length) {
    ...
    const snapshot = snapshotInventarioDesdeTabla().filter(
      (item) => item.recurso && item.categoria
    );

    if (snapshot.length) {
      data = snapshot;
      saveJSON(INVENTORY_KEY, snapshot);
    } else {
      data = [];
      saveJSON(INVENTORY_KEY, []);
    }
  }
...
function editarFila(button: HTMLButtonElement) {
  ...
  const thumbMarkup = renderThumbMarkup(original.imgSrc);
  celdas[5].innerHTML = `
    <div>
      ${thumbMarkup}
      <input type="file" class="editar-foto" accept="image/*" style="display:block; margin-top:6px;" />
    </div>`;
  celdas[5].dataset.foto = original.imgSrc.trim() ? "1" : "";
...
async function guardarFila(button: HTMLButtonElement) {
  ...
  celdas[5].innerHTML = renderThumbMarkup(fotoDataURL);
  celdas[5].dataset.foto = fotoDataURL.trim() ? "1" : "";
...
function cancelarEdicion(button: HTMLButtonElement) {
  ...
  celdas[5].innerHTML = renderThumbMarkup(original.imgSrc);
  celdas[5].dataset.foto = original.imgSrc.trim() ? "1" : "";
...
function updateEmptyState(totalCoincidencias: number) {
  const emptyState = document.getElementById("tablaVacia");
  ...
  const shouldShow = totalCoincidencias === 0;
  emptyState.toggleAttribute("hidden", !shouldShow);
  table.style.display = shouldShow ? "none" : "";

  if (paginacion) {
    paginacion.style.visibility = shouldShow ? "hidden" : "";
  }
}
